...
_() { case "${_ids-}" in (*$1*) ;; (*) _ids="$1,${_ids-}"; false;; esac; }; _ 1df8a1c && return 0
...
run_tests() {
  ...
  do
    ...
    log_file_path="$(mktemp "$TEMP_DIR"/XXXXXX)"
    local saved_flags
    saved_flags="$(set +o)"
  ...
  local opt=
  "$should_run_fulltest_80e79eb" && opt=--full
  if "$all" || test -n "$tests_to_run"
  then
    # shellcheck disable=SC2086
    if is_bbwin
    then
      echo Skipping Bash tests as the sh intepreter is BusyBox for Windows. >&2
    elif ! bash ./run-tests.bash $opt $tests_to_run
    then
      some_failed=true
    fi
  fi
