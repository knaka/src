...
test_script_embed() {
  ...
  init_temp_dir
  if sh "$scriptdir_0602a18"/testdata/original.sh | grep -q "BEGINNING"
  then
    echo 38a0c65 >&2
    false
  fi
  local temp_sh="$TEMP_DIR"/dc691d0.sh
  embed_minified_sub "$scriptdir_0602a18"/testdata/original.sh >"$temp_sh"
  sh "$temp_sh" | grep -q "BEGINNING"

  local temp_sh2="$TEMP_DIR"/6d0f004.sh
  embed_minified_sub "$scriptdir_0602a18"/testdata/original2.sh >"$temp_sh2"
  sh "$temp_sh2" | grep -q "Version: 123.45.6"

  local temp_sh3="$TEMP_DIR/7998a1a.sh"
  embed_minified_sub "$scriptdir_0602a18"/testdata/original3.sh >"$temp_sh3"
